...
class SingleKeyRange {
  ...
  static complement(keyRanges: SingleKeyRange[]): SingleKeyRange[] {
    ...
    for (let i = 0; i < complementKeyRanges.length; i++) {
      if (i === 0) {
        complementKeyRanges[i] = SingleKeyRange.upperBound(
          keyRanges[i].from as SingleKey,
          true
        );
      } else if (i === complementKeyRanges.length - 1) {
        complementKeyRanges[i] = SingleKeyRange.lowerBound(
          keyRanges[i - 1].to as SingleKey,
          true
        );
      } else {
...
class JoinPredicate extends PredicateNode {
  ...
  constructor(
    ...
    this.keyOfIndexFn = registry.getKeyOfIndexEvaluator(
      this.leftColumn.getType()
    );
  }
  ...
  reverse(): JoinPredicate {
    ...
    const newPredicate = new JoinPredicate(
      this.rightColumn,
      this.leftColumn,
      evaluatorType
    );
      ...
      for (let i = 0; i < leftEntriesLength; i++) {
          ...
          for (let j = currentBlock << blockNumBits; j < rightLimit; j++) {
            ...
            if (predicateResult) {
              ...
              const combinedEntry = RelationEntry.combineEntries(
                leftRelation.entries[i],
                leftRelationTables,
                rightRelation.entries[j],
                rightRelationTables
              );
        ...
        if (isOuterJoin && !matchFound) {
          combinedEntries.push(
            this.createCombinedEntryForUnmatched(
              leftRelation.entries[i],
              leftRelationTables
            )
          );
  ...
  evalRelationsHashJoin(
    ...
    maxRelation.entries.forEach(entry => {
      ...
      if (value !== null && map.has(key)) {
        ...
        if (isOuterJoin) {
          combinedEntries.push(
            this.createCombinedEntryForUnmatched(entry, maxRelationTableNames)
          );
        }
      }
    }, this);
  ...
  evalRelationsIndexNestedLoopJoin(
    ...
    if (
      leftRelation.getTables().indexOf(indexedTable.getEffectiveName()) !== -1
    ) {
  ...
  setComplement(isComplement: boolean): void {
    throw new Exception(
      ErrorCode.ASSERTION,
      'Join predicate has no complement'
    );
  }
  ...
  private createCombinedEntryForUnmatched(
    ...
    if (this.nullPayload === null) {
      this.nullPayload = this.createNullPayload(
        this.rightColumn.getTable() as BaseTable
      );
    }
    // The right relation is guaranteed to never be the result
    // of a previous join.
    const nullEntry = new RelationEntry(
      new Row(Row.DUMMY_ID, this.nullPayload),
      false
    );
    const combinedEntry = RelationEntry.combineEntries(
      entry,
...
class Journal {
  ...
  checkDeferredConstraints(): void {
    this.tableDiffs.forEach(tableDiff => {
      ...
      this.constraintChecker.checkForeignKeysForInsert(
        table,
        Array.from(tableDiff.getAdded().values()),
        ConstraintTiming.DEFERRABLE
      );
      this.constraintChecker.checkForeignKeysForDelete(
        table,
        Array.from(tableDiff.getDeleted().values()),
        ConstraintTiming.DEFERRABLE
      );
      this.constraintChecker.checkForeignKeysForUpdate(
        table,
        Array.from(tableDiff.getModified().values()),
        ConstraintTiming.DEFERRABLE
      );
    }, this);
  }
  ...
  private assertJournalWritable(): void {
    assert(
      ...
      'Attempted to use journal that needs to be rolled back.'
    );
  ...
  private removeByCascade(table: Table, deletedRows: Row[]): void {
    ...
    const cascadeDeletion = this.constraintChecker.detectCascadeDeletion(
      table as BaseTable,
      deletedRows
    );
...
class BundledObjectStore implements RuntimeTable {
  static forTableType(
    global: Global,
...
class IndexedDBRawBackStore implements RawBackStore {
...
class ObjectStore implements RuntimeTable {
  constructor(
    private store: IDBObjectStore,
    private deserializeFn: (raw: RawRow) => Row
  ) {}
...
class LocalStorage implements BackStore {
  private schema: DatabaseSchema;
...
class WebSqlTable implements RuntimeTable {
  private name: string;
  constructor(
    private tx: WebSqlTx,
    name: string,
...
class WebSqlRawBackStore implements RawBackStore {
